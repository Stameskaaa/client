...
type IntersectionintervalId = undefined | ReturnType<typeof setInterval>;

export const FriendRequest: React.FC<{ profileObj: UserData }> = ({ profileObj }) => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [indexState, setIndexState] = useState(0);
  const [requestList, setRequestList] = useState<UserData[]>([]);

  const cancelSubRequest = async (name: string) => {
    setLoading(true);
    try {
  ...
  const addFriendRequest = async (name: string) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    let intervalId: IntersectionintervalId;
        ...
        loading ? (
          ...
          requestList[indexState].img && (
            <div className={styles.container}>
              ...
              {requestList[indexState].lastName !== 'unknown'
                ? requestList[indexState].lastName
                : null}
              <div className={styles.container_button}>
                <button onClick={() => addFriendRequest(requestList[indexState].name)}>
                  &#10003;
                </button>
                <button
