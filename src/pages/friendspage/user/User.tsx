...
interface Props {
  personInfo: UserData;
  state: string;
  cancelSubscribe: (name: string) => void;
  deleteSubscriber: (name: string) => void;
  addSubscriber: (name: string) => void;
  users: UserData[];
  changedFriendList: (name: string) => void;
}
  ...
  return (
    <div className={styles.container_user}>
      ...
      {state === 'add' ? (
        ...
        <>
          {' '}
          <div className={styles.notification}>он подписался на тебя</div>
          <CiSquareMinus
          ...
          <IoIosCheckmarkCircleOutline
            onClick={() => addSubscriber(personInfo.name)}
